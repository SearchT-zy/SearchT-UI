 ...
 *      (e.g. out/linux-unpacked/SearchT-UI, out/mac-arm64/SearchT-UI.app, out/win-unpacked/SearchT-UI.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'searcht-e2e-state-'));
...
const e2eUserDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'searcht-e2e-userdata-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/SearchT-UI.exe  or  out/win-x64-unpacked/SearchT-UI.exe
    ...
    // out/mac-arm64/SearchT-UI.app/Contents/MacOS/SearchT-UI  or  out/mac/SearchT-UI.app/...
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    SEARCHT_EXTENSIONS_PATH: process.env.SEARCHT_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    SEARCHT_EXTENSION_STATES_FILE: process.env.SEARCHT_EXTENSION_STATES_FILE || e2eStateFile,
    SEARCHT_DISABLE_AUTO_UPDATE: '1',
    SEARCHT_DISABLE_DEVTOOLS: '1',
    SEARCHT_E2E_TEST: '1',
    SEARCHT_E2E_USER_DATA_DIR: process.env.SEARCHT_E2E_USER_DATA_DIR || e2eUserDataDir,
    /**
     ...
     */
    SEARCHT_CDP_PORT: process.env.SEARCHT_CDP_PORT || '9230',
  };
