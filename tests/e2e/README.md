...
cd ../AionCore && cargo install --path crates/searcht-app
...
| ----------------------------- | ----- | ------------------------ |
| `SEARCHT_E2E_TEST`            | `1`   | App recognizes test mode |
| `SEARCHT_DISABLE_AUTO_UPDATE` | `1`   | No update checks         |
| `SEARCHT_DISABLE_DEVTOOLS`    | `1`   | No DevTools windows      |
| `SEARCHT_CDP_PORT`            | `0`   | CDP disabled             |
...
```bash
# Clean via database
sqlite3 "~/Library/Application Support/SearchT-UI-Dev/aionui/aionui.db" \
