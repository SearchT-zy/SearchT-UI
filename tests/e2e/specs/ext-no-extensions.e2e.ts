...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'searcht-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'searcht-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      SEARCHT_EXTENSIONS_PATH: emptyExtensionsDir,
      SEARCHT_EXTENSION_STATES_FILE: extensionStatesFile,
