...
function resolveBackendBinary(): string {
  ...
  const candidates = [
    process.env.SEARCHT_BACKEND_BINARY,
...
function schemaPath(): string {
  return path.join(process.cwd(), '../aionCore/crates/searcht-db/migrations/001_initial_schema.sql');
}
...
test.describe('Assistant Settings Migration', () => {
  ...
  async function startBackend(): Promise<void> {
    ...
    backend = spawn(binary, ['--local', '--port', String(MIGRATION_BACKEND_PORT), '--data-dir', dataDir], {
      ...
      env: {
        ...process.env,
        RUST_LOG: 'warn',
        SEARCHT_EXTENSIONS_PATH: path.join(process.cwd(), 'examples'),
      },
    });
    try {
  ...
  test.beforeEach(async () => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'searcht-assistant-migration-'));
