...
/** Close the feedback modal (SearchtModal sets closable=false so Escape is a no-op). */
async function closeFeedbackModal(page: Page) {
  // The feedback modal is an SearchtModal (standard variant); its header close
...
/** Close any open SearchtModal (e.g. the Agent editor) so the next test starts clean. */
...
async function openCustomAgentEditor(page: Page, command: string) {
  // Defensive: close any SearchtModal left over from a prior test so the
...
test('[5] Agent fail_cli alert shows without feedback pill', async ({ page }) => {
  await openCustomAgentEditor(page, 'searcht-e2e-missing-binary-xyz');
