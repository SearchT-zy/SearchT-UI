...
test.describe('Agent Management Diagnostics — E2E', () => {
  test('settings agent page exposes diagnostics rows and hides legacy market actions', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="settings-tab-installed"]')).toHaveAttribute('aria-selected', 'true');
    await page.locator('[data-testid="settings-tab-all"]').click();
    await expect(page.locator('[data-testid="settings-tab-all"]')).toHaveAttribute('aria-selected', 'true');
    ...
    await Promise.all(
      managedAgents
        .slice(0, 4)
        .flatMap((agent) => [
          expect(page.locator(`[data-testid="agent-row-${agent.id}"]`)).toBeVisible({ timeout: 8_000 }),
          expect(page.locator(`[data-testid="agent-row-test-${agent.id}"]`)).toBeVisible({ timeout: 8_000 }),
          expect(page.locator(`[data-testid="agent-row-edit-${agent.id}"]`)).toBeVisible({ timeout: 8_000 }),
        ])
    );
  });

  test('agent row edit action opens the repair page', async ({ page }) => {
    const firstAgent = (await httpGet<ManagedAgent[]>(page, '/api/agents/management')).find(isVisibleManagedAgent);
    if (!firstAgent) {
  ...
  test('connection center remains usable at a narrow desktop size', async ({ electronApp, page }) => {
    const originalSize = await electronApp.evaluate(async ({ BrowserWindow }) => {
      const win = BrowserWindow.getAllWindows().find(
        (window) => !window.webContents.getURL().startsWith('devtools://')
      );
    ...
    try {
      await setWindowContentSize(electronApp, 980, 720);
      await goToSettings(page, 'agent');

      await expect(page.locator('[data-testid="agent-management-page"]')).toBeVisible({ timeout: 8_000 });
      await expect(page.locator('[data-testid="agent-check-installed"]')).toBeVisible();
      await expect(page.locator('[data-testid="agent-connection-summary"]')).toBeVisible();
      await expect(page.locator('[data-testid="settings-tab-installed"]')).toHaveAttribute('aria-selected', 'true');

      const horizontalOverflow = await page
        .locator('[data-testid="agent-management-page"]')
