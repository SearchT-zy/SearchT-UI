...
- **Builtin**：随 SearchT-UI 发布，不可删除，用户可导出到外部 CLI
- **Custom**：用户自行导入，可删除，可导出
- **Extension**：插件贡献，不可删除/导出（由插件管理）
- **Auto**：系统自动注入，无需用户选择（如 searcht-skills）
...
3. **Extension Skills 数据来源**?
   - E2E 环境的 `SEARCHT_EXTENSIONS_PATH` 已在 `fixtures.ts:112` 设置为 `examples/`
...
- Extension Skills:依赖 `SEARCHT_EXTENSIONS_PATH`(已在 fixtures.ts 配置)
...
1. **环境配置**
   - `SEARCHT_EXTENSIONS_PATH` 指向 `examples/`(已在 fixtures.ts 配置)
...
```typescript
// test-strategy § 3.2
const tempExternal = fs.mkdtempSync(path.join(os.tmpdir(), 'searcht-e2e-external-'));

// test-cases § 3.2
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'e2e-external-'));
```

**细微差异**: 前缀 `searcht-e2e-external-` vs `e2e-external-`
**建议**: 统一为 `searcht-e2e-external-` (test-strategy 的更明确)
...
- test-strategy § 3.2: `searcht-e2e-external-`
- test-cases § 3.2: `e2e-external-`

**影响**: 不影响功能，但统一命名可提升可读性和问题排查效率

**建议**: 在 test-strategy 或 test-cases 任一文档中明确统一为 `searcht-e2e-external-`，并让双方在实现时遵循
...
```markdown
**注意**：E2E 测试依赖 `SEARCHT_EXTENSIONS_PATH=examples/`（已在 `fixtures.ts:112` 配置）。
...
- `SEARCHT_EXTENSIONS_PATH` 环境变量（已在 E2E fixtures 中配置为 `examples/`）
...
- `_builtin/` 目录存在且包含技能（如 `searcht-skills`）
...
- `SEARCHT_EXTENSIONS_PATH` 环境变量指向 `examples/`
...
- `_builtin/` 目录存在且包含至少 1 个技能（如 `searcht-skills`）
...
✅ **前置条件明确**：`_builtin/` 目录存在（如 `searcht-skills`）
...
- ✅ 显示至少 1 个自动技能卡片（如 `searcht-skills`）
```
...
- Auto-injected skills 位于 `_builtin/` 目录，可选存在（如 `searcht-skills`）
