...
1. Extend `SEARCHT_E2E_TEST` to include worker ID:
   ```typescript
   SEARCHT_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
...
**Complexity**: High (requires main process to read `SEARCHT_DATABASE_PATH`, conflicts with userData convention)
