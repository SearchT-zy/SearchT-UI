...
/**
 * Searcht test models structure.
 */
export interface SearchtTestModels {
 ...
 */
export function resolveSearchtBinary(): string | null {
  try {
 ...
 */
export async function getSearchtTestModels(page: Page): Promise<SearchtTestModels | null> {
  try {
    const providers = (await httpGet<ProviderResponse[]>(page, '/api/providers')).map(normalizeProvider);
    if (!Array.isArray(providers)) return null;

    const isSearchtCompatible = (p: TProviderWithModel): boolean => {
    ...
    const candidates = providers.filter(
      (p) => p.enabled !== false && Array.isArray(p.model) && p.model.length > 0 && p.apiKey && isSearchtCompatible(p)
    );
 ...
 */
export async function resolveSearchtPreconditions(page: Page): Promise<{
  binary: string | null;
  models: SearchtTestModels | null;
}> {
  const binary = resolveSearchtBinary();
  const models = await getSearchtTestModels(page);
 ...
 */
export async function createSearchtConversationViaBridge(
  page: Page,
  opts: {
    name?: string;
    workspace: string;
  ...
  const name = opts.name || `E2E-searcht-${timestamp}`;
  ...
  if (!result?.id) {
    throw new Error(
      `createSearchtConversationViaBridge: bridge returned no conversation id — check provider apiKey and platform fields`
    );
  }
  return result.id;
}
 ...
 */
export async function sendSearchtMessage(
  page: Page,
 ...
 */
export async function waitForSearchtReply(page: Page, conversationId: string, timeoutMs = 150_000): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (aiTextMsgs.length > 0) {
      ...
      const conv = await getSearchtConversationDB(page, conversationId);
  ...
  const finalConv = await getSearchtConversationDB(page, conversationId);
  const finalMsgs = await getSearchtMessages(page, conversationId);
  console.error(`[waitForSearchtReply TIMEOUT] conv.status=${finalConv?.status}, msg count=${finalMsgs.length}`);
  for (const m of finalMsgs) {
    ...
    console.error(
      `[waitForSearchtReply TIMEOUT]   - pos=${m.position} type=${m.type} status=${m.status} preview="${preview}"`
    );
  }
  throw new Error(`Searcht reply timeout after ${timeoutMs}ms for conversation ${conversationId}`);
}
 ...
 */
export async function getSearchtConversationDB(page: Page, conversationId: string): Promise<any> {
  try {
 ...
 */
export async function getSearchtMessages(page: Page, conversationId: string): Promise<any[]> {
  try {
 ...
 * Deletes conversations with name pattern 'E2E-searcht-%'.
 * Throws error if deletion fails (no silent failures).
 * @param page Playwright page
 */
export async function cleanupE2ESearchtConversations(page: Page): Promise<void> {
  ...
  const e2eConversations = conversations.filter((conv: any) => conv.name?.startsWith('E2E-searcht-'));
...
export function createTempWorkspace(scenario: string): { path: string; cleanup: () => void } {
  const timestamp = Date.now();
  const dirPath = `/tmp/e2e-chat-searcht-${scenario}-${timestamp}`;
...
export async function selectSearchtAgent(page: Page): Promise<void> {
  await goToGuid(page);
 ...
 */
export async function selectSearchtModel(page: Page, modelId: string): Promise<void> {
  const modelBtn = page.locator('[data-testid="searcht-model-selector"]');
  await modelBtn.waitFor({ state: 'visible', timeout: 10_000 });
  await modelBtn.click();

  const modelOption = page.locator(`[data-testid="searcht-model-option-${modelId}"]`);
 ...
 */
export async function selectSearchtMode(page: Page, mode: string): Promise<void> {
  const modeBtn = page.locator('[data-testid="agent-mode-selector-aionrs"]');
  await modeBtn.waitFor({ state: 'visible', timeout: 10_000 });
  await modeBtn.click();

  const modeOption = page.locator(`[data-testid="searcht-mode-option-${mode}"]`);
 ...
 */
export async function attachSearchtFolder(page: Page, folderPath: string): Promise<void> {
  ...
  const attachBtn = page.locator('[data-testid="searcht-attach-folder-btn"]');
  await attachBtn.waitFor({ state: 'visible', timeout: 10_000 });

  // Skip actual click for smoke test (would trigger file dialog)
  console.log(`[attachSearchtFolder] Attach button found (folder: ${folderPath})`);
}
 ...
 */
export async function uploadSearchtFiles(page: Page, filePaths: string[]): Promise<void> {
  const fileInput = page.locator('[data-testid="searcht-file-upload-input"]');
  await fileInput.setInputFiles(filePaths);

  // Wait for file preview tags to appear
  await page.waitForSelector('[data-testid^="searcht-file-tag-"]', { timeout: 5_000 });
}
