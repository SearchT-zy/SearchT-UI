...
const GITHUB_RELEASES = [
  {
    ...
    name: 'v2.1.45',
    body: 'changelog body',
    html_url: 'https://github.com/SearchT-zy/SearchT/releases/tag/v2.1.45',
    prerelease: false,
    draft: false,
    assets: [],
  },
...
describe('update.check CDN-first', () => {
  ...
  it('reports an update from the CDN manifest and attaches GitHub notes', async () => {
    ...
    expect(res.data?.latest?.htmlUrl).toBe('https://github.com/SearchT-zy/SearchT/releases/tag/v2.1.45');
    expect(res.data?.latest?.recommendedAsset?.url).toBe(
      'https://github.com/SearchT-zy/SearchT/releases/download/2.1.45/SearchT-2.1.45-mac-arm64.dmg'
    );
  });
