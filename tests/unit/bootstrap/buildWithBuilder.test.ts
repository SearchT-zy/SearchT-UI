...
function readInstallerErrorDefinitions(): Array<{ defineName: string; code: string }> {
  ...
  return Array.from(source.matchAll(/!define\s+(SEARCHT_E_[A-Z0-9_]+)\s+"(E\d{4})"/g), (match) => ({
    defineName: match[1],
    code: match[2],
  }));
}
...
`,
  ...
  it('releases the NSIS output directory before any update repair or uninstall work', () => {
    ...
    const preInit = script.match(/!macro SEARCHT_INSTALLER_PREINIT([\s\S]*?)!macroend/)?.[1];
    const releaseMacro = script.match(/!macro SEARCHT_RELEASE_INSTALL_DIR_OUTDIR([\s\S]*?)!macroend/)?.[1];
    ...
    expect(preInit).toContain('!insertmacro SEARCHT_RELEASE_INSTALL_DIR_OUTDIR');
    expect(preInit!.indexOf('SEARCHT_RELEASE_INSTALL_DIR_OUTDIR')).toBeLessThan(
      preInit!.indexOf('SEARCHT_SESSION_BEGIN')
    );
  });
  ...
  it('uses install-directory ownership checks in the shared Windows NSIS include', () => {
    ...
    expect(script).not.toContain("Name -ieq '${SEARCHT_APP_EXECUTABLE_FILENAME}'");
  });
  ...
  it('records installer self-lock diagnostics when Restart Manager finds no locking process', () => {
    ...
    const captureMacro = script.match(/!macro SEARCHT_CAPTURE_FAILED_PATH_LOCKERS[\s\S]*?!macroend/)?.[0];

    expect(script).toContain('searcht-query-lockers.ps1');
    expect(captureMacro).toContain('SEARCHT_QUERY_LOCKERS');
    expect(captureMacro).not.toContain('SEARCHT_QUERY_LOCKERS_INLINE_LEGACY');
  ...
  it('continues with the bundled uninstaller when installed-uninstaller repair remains locked', () => {
    ...
    const retryFailureBranch = script.match(
      /\$\{If\} \$\{Errors\}\s+([\s\S]*?)\$\{Else\}\s+!insertmacro SEARCHT_LOG_UNINSTALLER_REPAIR "after-copy-retry"/
    )?.[1];

    expect(retryFailureBranch).toBeTruthy();
    expect(retryFailureBranch).toContain('copy-failed-using-bundled');
    expect(retryFailureBranch).toContain('$SearchtBundledUninstaller');
    expect(retryFailureBranch).not.toContain('MessageBox');
    expect(retryFailureBranch).not.toContain('SEARCHT_MSG_UNINSTALLER_LOCKED');
  ...
  it('keeps coded Windows installer failures on the unified reportable failure path', () => {
    ...
    for (const file of files) {
      ...
      source.split(/\r?\n/).forEach((line, index) => {
        if (line.includes('!macro SEARCHT_FAIL ')) {
          offenders.push(`${file}:${index + 1}: defines non-reportable coded failure macro`);
        }
        if (line.includes('!insertmacro SEARCHT_FAIL ')) {
  ...
  it('allows raw Windows installer MessageBox calls only for unified reporting or non-terminal prompts', () => {
    ...
    const allowedMessageBoxes = new Map<string, RegExp[]>([
      ...
      [
        ...
        [/SEARCHT_MSG_FILE_OR_FOLDER_IN_USE_ZH/, /\$\(appRunning\)/, /SEARCHT_MSG_CLOSE_OR_REMOVE_PREVIOUS_ZH/],
      ],
    ]);
  ...
  it('routes app-cannot-be-closed cancellation through E1003 instead of quitting silently', () => {
    ...
    const cannotCloseBranch = script.match(
      /SEARCHT_MSG_CLOSE_OR_REMOVE_PREVIOUS_ZH[\s\S]*?IDRETRY searcht_wait_for_close([\s\S]*?)\$\{Else\}/
    )?.[1];

    expect(cannotCloseBranch).toBeTruthy();
    expect(cannotCloseBranch).toContain('SEARCHT_E_INSTALL_DIR_REMOVE_OR_LOCKED');
    expect(cannotCloseBranch).toContain('SEARCHT_FAIL_REPORTABLE_BILINGUAL_DIAGNOSTICS');
...
function recordPrepareCall(options) {
  const callsPath = process.env.SEARCHT_PREPARE_CALLS_FILE;
...
`,
    ...
    try {
      ...
      const result = spawnSync(process.execPath, ['scripts/build-with-builder.js', ...args], {
        ...
        env: {
          ...process.env,
          SEARCHT_PREPARE_CALLS_FILE: callsPath,
          NODE_OPTIONS: [process.env.NODE_OPTIONS, `--require=${hookPath}`].filter(Boolean).join(' '),
        },
      });
