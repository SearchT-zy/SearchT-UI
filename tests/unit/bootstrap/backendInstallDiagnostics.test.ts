...
describe('collectBackendInstallDiagnostics', () => {
  it('records packaged runtime manifest and missing backend binary metadata', () => {
    const files = new Map<string, { mtimeMs: number; size: number; content?: string }>([
      ...
      ['C:\\AionUi\\resources\\bundled-backend\\win32-x64', { mtimeMs: 2000, size: 0 }],
      [
        'C:\\AionUi\\resources\\bundled-backend\\win32-x64\\manifest.json',
        {
          mtimeMs: 3000,
          size: 88,
          content: JSON.stringify({
            ...
            sourceType: 'download',
            files: ['searcht-backend.exe', 'managed-resources/'],
          }),
        },
      ],
    ]);
    ...
    const diagnostics = collectBackendInstallDiagnostics(
      {
        ...
        checkedBundledPath: 'C:\\AionUi\\resources\\bundled-backend\\win32-x64\\searcht-backend.exe',
      },
      {
        appVersion: '2.1.7',
        arch: 'x64',
    ...
    expect(diagnostics).toEqual({
      ...
      binaryExists: false,
      binaryName: 'searcht-backend.exe',
      binaryPath: 'C:\\AionUi\\resources\\bundled-backend\\win32-x64\\searcht-backend.exe',
      bundledDirPath: 'C:\\AionUi\\resources\\bundled-backend',
      execPath: 'C:\\AionUi\\AionUi.exe',
      isPackaged: true,
      manifestExists: true,
      manifestFiles: ['searcht-backend.exe', 'managed-resources/'],
      manifestGeneratedAt: '2026-05-29T12:00:00.000Z',
      manifestPath: 'C:\\AionUi\\resources\\bundled-backend\\win32-x64\\manifest.json',
      manifestSize: 88,
      ...
      runtimeDirPath: 'C:\\AionUi\\resources\\bundled-backend\\win32-x64',
      runtimeKey: 'win32-x64',
    });
  });
});
