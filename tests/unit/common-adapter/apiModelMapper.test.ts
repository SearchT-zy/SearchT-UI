  ...
  describe('buildCreateConversationBody', () => {
    ...
    it('passes conversation skill fields through verbatim', () => {
      ...
      const assistant = body.assistant as {
        conversation_overrides: { skill_ids: string[]; disabled_builtin_skill_ids: string[] };
      };
      expect(assistant.conversation_overrides.skill_ids).toEqual(['aionui-config', 'searcht-app-guide']);
      expect(assistant.conversation_overrides.disabled_builtin_skill_ids).toEqual(['aionui-troubleshooting']);
