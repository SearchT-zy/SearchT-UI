...
const appMock = vi.hoisted(() => ({
  ...
  getPath: vi.fn(() => '/tmp/searcht-test'),
  exit: vi.fn(),
}));
...
describe('AutoUpdaterService', () => {
  ...
  beforeEach(() => {
    ...
    appMock.getPath.mockImplementation(() => '/tmp/searcht-test');
    delete (autoUpdaterMock as { updateInfoAndProvider?: unknown }).updateInfoAndProvider;
    appMock.isPackaged = false;
    delete process.env.SEARCHT_FORCE_DEV_AUTO_UPDATE;
    delete process.env.SEARCHT_DEBUG_AUTO_UPDATE_CURRENT_VERSION;
  ...
  it('configures only the explicit SearchT-UI update feed', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenCalledWith({
      provider: 'custom',
      url: 'https://updates.searcht.example/releases',
  ...
  it('skips the startup notify check when the update service is not configured', async () => {
    delete process.env.SEARCHT_UPDATE_BASE_URL;
    const { autoUpdaterService } = await import('@/process/services/autoUpdaterService');
  ...
  it('enables forced updater checks in unpacked dev builds when requested', async () => {
    process.env.SEARCHT_FORCE_DEV_AUTO_UPDATE = '1';
  ...
  it('overrides the updater current version only for forced unpacked dev checks', async () => {
    process.env.SEARCHT_FORCE_DEV_AUTO_UPDATE = '1';
    process.env.SEARCHT_DEBUG_AUTO_UPDATE_CURRENT_VERSION = '2.1.12';
  ...
  it('moves the process cwd to temp before the Windows updater handoff', async () => {
    setPlatform('win32');
    const tempRoot = path.join(process.env.TEMP || process.cwd(), `searcht-updater-cwd-test-${process.pid}`);
    const expectedCwd = path.join(tempRoot, 'searcht-updater-cwd');
    const chdir = vi.spyOn(process, 'chdir').mockImplementation(() => undefined);
    appMock.getPath.mockImplementation((name: string) => (name === 'temp' ? tempRoot : '/tmp/searcht-test'));
