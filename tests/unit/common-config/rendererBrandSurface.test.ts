...
describe('SearchT renderer brand surface', () => {
  it('uses SearchT in static application metadata', () => {
    const html = fs.readFileSync(path.join(rendererRoot, 'index.html'), 'utf8');

    expect(html).toContain('<meta name="application-name" content="SearchT" />');
    expect(html).toContain('<meta name="apple-mobile-web-app-title" content="SearchT" />');
    expect(html).toContain('<title>SearchT</title>');
  });

  it('uses SearchT on the login surface in every supported language', () => {
    for (const locale of i18nConfig.supportedLanguages) {
      ...
      expect(resource.brand, locale).toBe('SearchT');
      expect(resource.pageTitle, locale).toContain('SearchT');
      expect(resource.pageTitle, locale).not.toContain('AionUi');
    }
  });

  it('uses the shared SearchT name in the desktop sidebar', () => {
    ...
    expect(layout).not.toMatch(/>\s*SearchT\s*</);
