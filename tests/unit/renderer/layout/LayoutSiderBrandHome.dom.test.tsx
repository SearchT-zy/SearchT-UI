...
import { SEARCHT_DISPLAY_NAME } from '@/common/config/brandIdentity';
...
describe('Layout sider brand Home button', () => {
  ...
  it('navigates to the recorded last non-settings path when clicked in a settings route', () => {
    ...
    sessionStorage.setItem('searcht:last-non-settings-path', '/conversation/abc');
    renderLayout();
  ...
  it('falls back to /guid when the recorded path is itself a settings path', () => {
    currentPathname = '/settings/about';
    sessionStorage.setItem('searcht:last-non-settings-path', '/settings/system');
    renderLayout();
  ...
  it('renders the wordmark as a non-actionable element in a non-settings route', () => {
    ...
    const wordmark = screen.getByText(SEARCHT_DISPLAY_NAME);
  ...
  it('does not navigate when the wordmark is clicked in a non-settings route', () => {
    ...
    fireEvent.click(screen.getByText(SEARCHT_DISPLAY_NAME));
    expect(navigate).not.toHaveBeenCalled();
  });
  ...
  it('opens the update notification directly for tray update checks', () => {
    ...
    window.addEventListener('searcht-open-update-modal', openListener);
    ...
    try {
      ...
      window.removeEventListener('searcht-open-update-modal', openListener);
    }
  });
});
