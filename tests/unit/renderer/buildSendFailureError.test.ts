...
describe('buildSendFailureError', () => {
  it('classifies 409 already-processing as SEARCHT_CONVERSATION_BUSY (wait, not retry)', () => {
    ...
    expect(result).toEqual({
      ...
      code: 'SEARCHT_CONVERSATION_BUSY',
      ownership: 'searcht',
  ...
  it('classifies 409 already-running as SEARCHT_CONVERSATION_BUSY', () => {
  ...
  it('classifies runtime shutdown conflict as non-retryable busy without raw internal diagnostics', () => {
    ...
    expect(result.code).toBe('SEARCHT_CONVERSATION_BUSY');
  ...
  it('falls back to SEARCHT_INTERNAL_ERROR for non-conflict 409 (different message)', () => {
    ...
    expect(result.code).toBe('SEARCHT_INTERNAL_ERROR');
    expect(result.retryable).toBe(true);
  });

  it('falls back to SEARCHT_INTERNAL_ERROR for non-HTTP errors', () => {
    const result = buildSendFailureError(new Error('boom'), 'boom');

    expect(result.code).toBe('SEARCHT_INTERNAL_ERROR');
    expect(result.ownership).toBe('searcht');
    expect(result.retryable).toBe(true);
  });
