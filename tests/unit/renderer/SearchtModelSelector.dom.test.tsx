...
import type { SearchtModelSelection } from '@/renderer/pages/conversation/platforms/aionrs/useSearchtModelSelection';
...
const makeSelection = (overrides: Partial<SearchtModelSelection> = {}): SearchtModelSelection => ({
  current_model: {
    ...provider,
    use_model: 'gpt-5.2',
...
describe('SearchtModelSelector runtime options', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows the current model and thought level in the header pill', () => {
    ...
    expect(screen.getByTestId('searcht-model-selector')).toHaveTextContent('gpt-5.2 · High');
  });
  ...
  it('renders the model list directly (no submenu) when thought level is unavailable', () => {
    render(<SearchtModelSelector selection={makeSelection()} />);

    expect(screen.getByTestId('searcht-model-selector')).toHaveTextContent('gpt-5.2');
