...
import { useSearchtMessage } from '@/renderer/pages/conversation/platforms/aionrs/useSearchtMessage';
...
describe('useSearchtMessage turn clock', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('preserves the turn start timestamp when switching away and back to a running conversation', async () => {
    ...
    const { result, rerender } = renderHook(({ id }) => useSearchtMessage(id), {
  ...
  it('keeps the persisted origin when entering a running conversation from an idle one', async () => {
    ...
    const nowSpy = vi.spyOn(Date, 'now').mockReturnValue(100_000);
    const first = renderHook(() => useSearchtMessage('conv-2'));
    await waitFor(() => {
  ...
  it('clears the turn start timestamp when the turn finishes', async () => {
    ...
    const { result } = renderHook(() => useSearchtMessage('conv-1'));
    await waitFor(() => {
      expect(result.current.hasHydratedRunningState).toBe(true);
    });
