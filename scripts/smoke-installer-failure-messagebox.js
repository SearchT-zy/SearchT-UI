...
const INSTALLER_ERROR_SCENARIOS = [
  ...
  },
  {
    ...
    defineName: 'SEARCHT_E_INVALID_INSTALL_PATH',
    code: 'E1090',
...
function copyHarnessProject(projectRoot) {
  ...
  writeFileSync(path.join(supportDir, '_sentry-dsn.generated.nsh'), '!define SEARCHT_SENTRY_DSN ""\n', 'utf8');
}
...
function readInstallerErrorDefinitions() {
  ...
  const definitions = Array.from(source.matchAll(/!define\s+(SEARCHT_E_[A-Z0-9_]+)\s+"(E\d{4})"/g), (match) => ({
    defineName: match[1],
    code: match[2],
  }));
  if (definitions.length === 0) {
    throw new Error('No SEARCHT_E_* installer error codes found.');
  }
  return definitions;
}
...
function runHarness({ autoDecline, compileOnly, makensis, scenario }) {
  const { code } = scenario;
  const root = mkdtempSync(path.join(tmpdir(), `searcht-failure-messagebox-${code}-`));
  const projectRoot = path.join(root, 'project');
  const nsiPath = path.join(root, 'searcht-failure-messagebox-smoke.nsi');
  const exePath = path.join(root, 'searcht-failure-messagebox-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `searcht-installer-messagebox-smoke-${code}-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}-log.jsonl`
  );
  const automationPath = path.join(root, 'auto-decline.ps1');
  const reportStatusPath = path.join(process.env.TEMP || tmpdir(), 'searcht-installer-report.json');
