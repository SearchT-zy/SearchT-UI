...
function patchElectronBuilderNsisInstaller() {
  ...
  const copiedUninstallerExecWithLog = `ExecWait '"$uninstallerFileNameTemp" /S /KEEP_APP_DATA $0 --installer-log="$SearchtSessionLogPath" --installer-session="$SearchtSessionId" _?=$installationDir' $R0`;
  // The cached app-builder-lib template may carry a legacy variable name from
  // a previous patch pass; normalize any known variants before applying.
  ...
  for (const variant of legacyLogVariants) {
    patched = patched
      .split(variant)
      .join(`--installer-log="$SearchtSessionLogPath" --installer-session="$SearchtSessionId"`);
  }
  if (patched.includes(copiedUninstallerExec)) {
    patched = patched.replace(copiedUninstallerExec, copiedUninstallerExecWithLog);
  } else if (
    patched.includes(
      `ExecWait '"$uninstallerFileNameTemp" /S /KEEP_APP_DATA $0 --installer-log="$SearchtSessionLogPath" _?=$installationDir' $R0`
    )
  ) {
  ...
  const inPlaceUninstallerExecWithLog = `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$SearchtSessionLogPath" --installer-session="$SearchtSessionId" _?=$installationDir' $R0`;
  if (patched.includes(inPlaceUninstallerExec)) {
    patched = patched.replace(inPlaceUninstallerExec, inPlaceUninstallerExecWithLog);
  } else if (
    patched.includes(
      `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$SearchtSessionLogPath" _?=$installationDir' $R0`
    )
  ) {
    patched = patched.replace(
      `ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 --installer-log="$SearchtSessionLogPath" _?=$installationDir' $R0`,
...
function writeGeneratedSentryDsnInclude(projectRoot) {
  ...
  fs.writeFileSync(
    generatedInclude,
    `!define SEARCHT_SENTRY_DSN "${escapeNsisDefineValue(process.env.SENTRY_DSN || '')}"\n`
  );
}
