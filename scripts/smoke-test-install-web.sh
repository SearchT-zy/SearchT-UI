...
export INSTALL_DIR="/tmp/searcht-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/searcht-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/searcht-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/searcht-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/searcht-web"
    exit 1
fi
...
VERSION_OUTPUT=$(searcht-web version 2>&1 || echo "")
