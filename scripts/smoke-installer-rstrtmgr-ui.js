...
function main() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'searcht-rm-ui-'));
  ...
  const exePath = path.join(root, 'searcht-rstrtmgr-ui-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `searcht-installer-smoke-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}.log`
  );
