...
const tarballName = `searcht-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'searcht-web');
...
const executableName = platform === 'win32' ? 'searcht-web.exe' : 'searcht-web';
...
// `searcht-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} searcht-web`, {
