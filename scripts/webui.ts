 ...
 *
 * Env vars:
 *   SEARCHT_PORT           : static server port (default 33000)
 *   SEARCHT_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   SEARCHT_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 *   SEARCHT_DATA_DIR       : override userData path (default Electron-compatible)
 *   SEARCHT_LOG_DIR        : override log dir (default <dataDir>/logs)
 *   SEARCHT_STATIC_DIR     : override static dir (default out/renderer)
 *   SEARCHT_BACKEND_BIN    : absolute path to backend binary (else PATH lookup)
 *   SEARCHT_BACKEND_BUNDLED_DIR : dir containing bundled-backend/<plat-arch>/binary
 *   SEARCHT_OPEN_BROWSER   : "1"/"true" to force open, "0"/"false" to disable
 */
...
import { ensureSkillStagingDirectory } from '../packages/desktop/src/process/services/personal-core/content/skills/skillStaging';

// Aligned with packages/desktop/src/common/config/constants.ts WEBUI_DEFAULT_PORT.
const DEFAULT_PORT = (() => {
  if (process.env.NODE_ENV === 'production') return 25808;
  if (process.env.SEARCHT_MULTI_INSTANCE === '1') return 25810;
  return 25809;
})();
...
const backendBinaryNames = (process.platform === 'win32' ? ['searcht-backend.exe', 'aioncore.exe'] : ['searcht-backend', 'aioncore']);
 ...
 *   $SEARCHT_DATA_DIR        env override (same effect)
 *   otherwise               ~/.searcht-web         (production)
 *                           ~/.searcht-web-dev     (dev, default)
 *                           ~/.searcht-web-dev-2   (dev + SEARCHT_MULTI_INSTANCE=1)
 *
 ...
 * `.searcht-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.searcht-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.SEARCHT_DATA_DIR;
  ...
  const suffix =
    process.env.NODE_ENV === 'production' ? '' : process.env.SEARCHT_MULTI_INSTANCE === '1' ? '-dev-2' : '-dev';
  const dir = path.join(os.homedir(), `.searcht-web${suffix}`);
...
function resolvePort(): number {
  ...
  const env = process.env.SEARCHT_PORT ?? process.env.PORT;
...
function resolveAllowRemote(): boolean {
  ...
  const host = process.env.SEARCHT_HOST?.trim();
  if (host && ['0.0.0.0', '::', '::0'].includes(host)) return true;
  return parseBoolean(process.env.SEARCHT_ALLOW_REMOTE ?? process.env.SEARCHT_REMOTE);
}

function resolveStaticDir(): string {
  if (process.env.SEARCHT_STATIC_DIR) return process.env.SEARCHT_STATIC_DIR;
  const candidate = path.join(repoRoot, 'out', 'renderer');
  if (fs.existsSync(path.join(candidate, 'index.html'))) return candidate;
  throw new Error(`Renderer assets not found at ${candidate}. Run "bun run package" first, or set SEARCHT_STATIC_DIR.`);
}
 ...
 *   $SEARCHT_NO_BUILD=1        : env-level opt-out
 *   $SEARCHT_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.SEARCHT_NO_BUILD)) return;
  if (process.env.SEARCHT_STATIC_DIR) return;
...
function resolveBackendBinary(): string {
  ...
  for (const binaryName of backendBinaryNames) {
  ...
  throw new Error(
    `Cannot find "${backendBinaryNames[0]}". Set SEARCHT_BACKEND_BIN, put it on PATH, or place it at ${path.join(bundledBase, runtimeKey, backendBinaryNames[0])}.`
  );
}
...
async function main(): Promise<void> {
  ...
  const logDir = process.env.SEARCHT_LOG_DIR ?? path.join(workDir, 'logs');
