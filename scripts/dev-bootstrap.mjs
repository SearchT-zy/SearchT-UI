...
const KILLABLE_NAMES = new Set(['electron', 'searcht-ui', 'searcht-ui.exe']);
...
function cleanupByName() {
  ...
  for (const proc of processes) {
    ...
    if (!['electron', 'searcht-ui'].some((k) => rawName.includes(k))) continue;
    if (killPid(pid)) {
...
function launch(scriptName, withExtensions) {
  ...
  if (withExtensions) {
    env.SEARCHT_EXTENSIONS_PATH = path.resolve(process.cwd(), 'examples');
    log(`SEARCHT_EXTENSIONS_PATH=${env.SEARCHT_EXTENSIONS_PATH}`);
  }
