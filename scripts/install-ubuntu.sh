...
#   # 或指定版本：
#   SEARCHT_VERSION=1.8.25 bash install-ubuntu.sh
#   # 僅安裝桌面版（跳過 headless 設定）：
#   SEARCHT_MODE=desktop bash install-ubuntu.sh
# ============================================================================
...
# ─── 取得版本號 ──────────────────────────────────────────────────────────────
resolve_version() {
    if [[ -n "${SEARCHT_VERSION:-}" ]]; then
        VERSION="$SEARCHT_VERSION"
        ...
        if [[ -z "$VERSION" ]]; then
            die "無法取得最新版本號，請手動指定: SEARCHT_VERSION=1.8.25 bash $0"
        fi
...
PIDFILE="/var/run/searcht-ui.pid"
LOGFILE="/var/log/searcht-ui.log"
WORKDIR="${SEARCHT_WORKDIR:-$HOME}"
...
create_systemd_service() {
    ...
    local service_path="/etc/systemd/system/searcht-ui.service"
...
SERVICE_EOF
    ...
    info "使用方式:"
    echo "    sudo systemctl start searcht-ui     # 啟動"
    echo "    sudo systemctl stop searcht-ui      # 停止"
    echo "    sudo systemctl enable searcht-ui    # 開機自動啟動"
    echo "    sudo systemctl status searcht-ui    # 查看狀態"
    echo "    journalctl -u searcht-ui -f         # 查看日誌"
}

# ─── 建立桌面捷徑 ─────────────────────────────────────────────────────────────
create_desktop_entry() {
    local desktop_dir="${HOME}/.local/share/applications"
    local desktop_file="${desktop_dir}/searcht-ui.desktop"
...
print_summary() {
    ...
    if [[ "${MODE}" == "headless" ]]; then
        echo -e "  ${YELLOW}💡 提示:${NC}"
        echo "     • 設定工作目錄: export SEARCHT_WORKDIR=/path/to/workspace"
