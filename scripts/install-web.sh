#!/usr/bin/env bash
# ============================================================================
# SearchT-UI WebUI — One-Click Installation Script
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/searcht-ui/SearchT-UI/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/searcht-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/searcht-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/searcht-ui/SearchT-UI/releases/download}"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║     SearchT-UI WebUI Installer (No Electron)     ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/searcht-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="searcht-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/searcht-ui/SearchT-UI/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is searcht-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/searcht-web" ]]; then
        mv "${extract_temp}/searcht-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing searcht-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/searcht-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/searcht-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/searcht-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/searcht-web"
    local target_path="${INSTALL_DIR}/searcht-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by searcht-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 SearchT-UI WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/searcht-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start SearchT-UI WebUI"
        echo "    searcht-web start"
        echo ""
        echo "    # Check version"
        echo "    searcht-web version"
    else
        echo "    # Start SearchT-UI WebUI (using full path)"
        echo "    ${INSTALL_DIR}/searcht-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/searcht-web ~/.local/bin/searcht-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/searcht-ui/SearchT-UI"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/searcht-ui/SearchT-UI/issues"
    echo ""
