 ...
 *   SEARCHT_DATA_DIR=/x bun run resetpass
 ...
 * See the comment there for why the default is `~/.searcht-web*` (not `~/.aionui*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `SEARCHT_PORT` to match
 ...
 * pass --port / SEARCHT_PORT if they launched webui on a non-default one.
 */
