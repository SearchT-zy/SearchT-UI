...
function main() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'searcht-self-lock-'));
  const installDir = path.join(root, 'install-dir');
  mkdirSync(installDir, { recursive: true });
  writeFileSync(path.join(installDir, 'existing-file.txt'), 'self-lock smoke\n', 'utf8');

  const nsiPath = path.join(root, 'searcht-self-lock-smoke.nsi');
  const exePath = path.join(root, 'searcht-self-lock-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `searcht-installer-self-lock-${new Date()
      .toISOString()
      .replace(/[-:]/g, '')
      .replace(/\..+$/, '')
      .replace('T', '-')}-log.jsonl`
  );
  const resultPath = path.join(process.env.TEMP || tmpdir(), `searcht-installer-self-lock-${process.pid}-result.txt`);
