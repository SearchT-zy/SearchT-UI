...
Create `start-searcht-webui.bat`:
...
```bash
chmod +x start-searcht-webui.sh
./start-searcht-webui.sh
```
...
Create `~/.local/share/applications/searcht-webui.desktop`:
...
```bash
chmod +x ~/.local/share/applications/searcht-webui.desktop
```
...
Create `~/bin/start-searcht-webui.sh`:
...
```bash
chmod +x ~/bin/start-searcht-webui.sh
```
...
Create `/etc/systemd/system/searcht-webui.service`:
...
sudo systemctl enable searcht-webui.service
sudo systemctl start searcht-webui.service

# Check status
sudo systemctl status searcht-webui.service
```
...
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/SearchT-zy/SearchT/issues/217)
...
# Check latest version at: https://github.com/SearchT-zy/SearchT/releases
wget https://github.com/SearchT-zy/SearchT/releases/download/vVERSION/SearchT_VERSION_arm64.deb
...
2. Report issues at [GitHub Issues #217](https://github.com/SearchT-zy/SearchT/issues/217)
...
export SEARCHT_PORT=8080

# Allow remote access without passing --remote
export SEARCHT_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as SEARCHT_ALLOW_REMOTE=true)
...
- [GitHub Issues](https://github.com/SearchT-zy/SearchT/issues)
...
2. Search [existing issues](https://github.com/SearchT-zy/SearchT/issues)
3. Create a [new issue](https://github.com/SearchT-zy/SearchT/issues/new) with:
   - Your OS and version
   - SearchT version
   - Steps to reproduce
