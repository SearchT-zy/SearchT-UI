...
The desktop development server resolves the backend from the `PATH` inherited by `bun run start`. Install AionCore first, verify the binary is discoverable in the same terminal, then start SearchT.
...
# Verify that SearchT will be able to find the backend.
which aioncore
aioncore --help
```
...
Run these commands from the `SearchT` repository in a terminal where `aioncore` is discoverable.
...
During startup, SearchT launches `aioncore` automatically and passes the backend port to the renderer. You do not need to start AionCore in a separate terminal.

## Updating the Local Backend

When you pull or change AionCore, reinstall the backend binary and restart SearchT:
...
SearchT cannot find the backend from the `PATH` inherited by `bun run start`.

Check from the same terminal where you start SearchT:
...
If the command fails, add Cargo's binary directory to `PATH` and start SearchT from a new terminal.

### `aioncore` Works in a Terminal but SearchT Still Cannot Find It
...
Quit SearchT, reinstall AionCore with `cargo install --path crates/searcht-app --locked --force`, then start SearchT again. The Electron app owns the backend subprocess during development, so a running SearchT instance will not pick up a newly installed binary until it restarts.
...
When you have two clones of the repository (e.g. `SearchT` and `SearchT-refactor`) and need to run both simultaneously, the second instance can be started with:
...
- Uses a separate userData directory (`SearchT-Dev-2`) to avoid database and config conflicts
...
SearchT uses **electron-vite** for fast bundling:
