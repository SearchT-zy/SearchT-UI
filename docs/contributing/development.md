...
SearchT-UI development uses two repositories:
...
- **SearchT-UI** (`https://github.com/searcht-ui/SearchT-UI.git`) starts the Electron desktop app and launches the backend binary automatically.
...
The desktop development server resolves the backend from the `PATH` inherited by `bun run start`. Install AionCore first, verify the binary is discoverable in the same terminal, then start SearchT-UI.
...
git clone https://github.com/searcht-ui/SearchT-UI.git
```
...
cd AionCore
cargo clean
cargo install --path crates/searcht-app --locked

# Make Cargo-installed binaries visible to this shell if needed.
export PATH="$HOME/.cargo/bin:$PATH"

# Verify that SearchT-UI will be able to find the backend.
which aioncore
aioncore --help
```
...
Run these commands from the `SearchT-UI` repository in a terminal where `aioncore` is discoverable.
...
During startup, SearchT-UI launches `aioncore` automatically and passes the backend port to the renderer. You do not need to start AionCore in a separate terminal.

## Updating the Local Backend

When you pull or change AionCore, reinstall the backend binary and restart SearchT-UI:

```bash
cd ../AionCore
cargo install --path crates/searcht-app --locked --force
...
SearchT-UI cannot find the backend from the `PATH` inherited by `bun run start`.

Check from the same terminal where you start SearchT-UI:
...
If the command fails, add Cargo's binary directory to `PATH` and start SearchT-UI from a new terminal.

### `aioncore` Works in a Terminal but SearchT-UI Still Cannot Find It
...
Quit SearchT-UI, reinstall AionCore with `cargo install --path crates/searcht-app --locked --force`, then start SearchT-UI again. The Electron app owns the backend subprocess during development, so a running SearchT-UI instance will not pick up a newly installed binary until it restarts.
...
When you have two clones of the repository (e.g. `SearchT-UI` and `SearchT-UI-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `SEARCHT_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`SearchT-UI-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.searcht-dev-2`, `~/.searcht-config-dev-2`)
...
SearchT-UI uses **electron-vite** for fast bundling:
