...
| 文件 | 平台 |
| --- | --- |
| `SearchTUI-<版本>-win-x64.exe` | Windows 10/11 x64（NSIS 安装器） |

安装后首次启动会引导完成六步设置。macOS / Linux 用户请从源码构建（见下）。
...
| 群组协作   | 本机 Agent 群聊：Leader 协调员拆解目标、分派任务给成员并汇总结果（详见 [智能体群组指南](docs/guide/agent-teams.md)）；@提及与广播、任务阻塞关系、排队与失败恢复 |
