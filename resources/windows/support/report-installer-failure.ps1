...
$log = $LogPath
if (-not $log) {
  $log = Join-Path $env:TEMP 'searcht-installer-fallback-log.jsonl'
}
...
function Get-OptionalHandleDiagnostics {
  ...
  $outPath = Join-Path $env:TEMP ('searcht-handle-' + [guid]::NewGuid().ToString('N') + '.out')
  $errPath = Join-Path $env:TEMP ('searcht-handle-' + [guid]::NewGuid().ToString('N') + '.err')
