...
import { cleanupRegisteredAgentProcesses } from './agent-process-registry';
import type { AppMetadata, BackendBinaryResolver } from './types';
...
export type BackendLaunchOptions = {
  ...
  logDir?: string;
  /**
   * System dirs exposed to the backend via SEARCHT_{CACHE,WORK,LOG}_DIR env.
...
export function buildSpawnArgs(config: SpawnConfig): string[] {
  const logLevel = process.env.SEARCHT_LOG_LEVEL || (config.isPackaged ? 'info' : 'debug');
  const args = [
    '--port',
    String(config.port),
    '--data-dir',
  ...
  if (!config.isPackaged && process.env.SEARCHT_DUMP_PROMPTS === '1') args.push('--dump-prompts');
...
export function buildSpawnEnv(dirs: BackendDirConfig): NodeJS.ProcessEnv {
  return {
    ...process.env,
    SEARCHT_CACHE_DIR: dirs.cacheDir,
    SEARCHT_WORK_DIR: dirs.workDir,
    SEARCHT_LOG_DIR: dirs.logDir,
  };
}
