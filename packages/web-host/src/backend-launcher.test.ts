...
describe('buildSpawnArgs', () => {
  ...
  it('passes prompt dump flag in development only when SEARCHT_DUMP_PROMPTS is enabled', () => {
    const prev = process.env.SEARCHT_DUMP_PROMPTS;
    process.env.SEARCHT_DUMP_PROMPTS = '1';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.SEARCHT_DUMP_PROMPTS;
      else process.env.SEARCHT_DUMP_PROMPTS = prev;
    }
  });
  ...
  it('respects SEARCHT_LOG_LEVEL override', () => {
    const prev = process.env.SEARCHT_LOG_LEVEL;
    process.env.SEARCHT_LOG_LEVEL = 'trace';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.SEARCHT_LOG_LEVEL;
      else process.env.SEARCHT_LOG_LEVEL = prev;
    }
  });
});

describe('buildSpawnEnv', () => {
  it('merges process.env with SEARCHT_* dir vars', () => {
    const env = buildSpawnEnv({
      cacheDir: '/c',
      workDir: '/w',
      logDir: '/l',
    });
    expect(env.SEARCHT_CACHE_DIR).toBe('/c');
    expect(env.SEARCHT_WORK_DIR).toBe('/w');
    expect(env.SEARCHT_LOG_DIR).toBe('/l');
...
describe('BackendLifecycleManager.start (success path)', () => {
  ...
  it('spawns with correct args, waits for /health, reports running', async () => {
    ...
    try {
      ...
      expect(opts.env.SEARCHT_CACHE_DIR).toBe('/c');
      expect(opts.env.SEARCHT_WORK_DIR).toBe('/w');
      expect(opts.env.SEARCHT_LOG_DIR).toBe('/l');
