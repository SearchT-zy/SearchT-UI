 ...
 *
 * On first tarball launch, the backend's SQLite `users` table holds the
...
export type EnsureAdminPasswordOptions = {
  /** 127.0.0.1 port where the backend listens (from WebHostHandle.backendPort). */
  backendPort: number;
   ...
   * by launch context — packaged tarball = `searcht-web resetpass`, in-repo dev
...
export async function ensureAdminPassword(
  ...
  const resetCmd = opts.resetCommand ?? 'searcht-web resetpass';
  ...
  try {
    ...
    deps.warn(`[searcht-web] could not verify admin credentials: ${err instanceof Error ? err.message : String(err)}`);
    return;
  }
  ...
  if (!needsSetup) {
    ...
    deps.log(`[searcht-web] Log in with username "${username}". Forgot the password? Run \`${resetCmd}\`.`);
    return;
  }
  ...
  try {
    ...
    if (!resetRes.ok) {
      deps.warn(`[searcht-web] /api/webui/reset-password returned ${resetRes.status} — run \`${resetCmd}\``);
      return;
    }
    ...
    if (!newPassword) {
      deps.warn(`[searcht-web] /api/webui/reset-password returned no new_password — run \`${resetCmd}\``);
      return;
    }
    const username = await fetchAdminUsername(deps, opts.backendPort);
    deps.log(`[searcht-web] Generated initial admin password: ${newPassword}`);
    deps.log(`[searcht-web] Log in with username "${username}" and change it from the UI.`);
  } catch (err) {
    deps.warn(
      `[searcht-web] failed to seed initial admin password: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
