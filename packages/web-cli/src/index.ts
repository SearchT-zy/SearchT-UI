...
function resolveCliRoot(): string {
  // Heuristic: if the executable path ends in "searcht-web" or "searcht-web.exe",
  // treat it as the packaged single-file binary and return its directory.
  const exe = process.execPath;
  const exeName = path.basename(exe).toLowerCase();
  if (exeName === 'searcht-web' || exeName === 'searcht-web.exe') {
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.SEARCHT_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.searcht-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.SEARCHT_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.SEARCHT_ALLOW_REMOTE ?? process.env.SEARCHT_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[searcht-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[searcht-web] version    : ${version}`);
  console.log(`[searcht-web] data dir   : ${dataDir}`);
  console.log(`[searcht-web] log dir    : ${logDir}`);
  console.log(`[searcht-web] static dir : ${staticDir}`);
  console.log(`[searcht-web] backend bin: ${backendBin}`);
  console.log(`[searcht-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.log('');
    console.log('SearchT-UI WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[searcht-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[searcht-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[searcht-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[searcht-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `searcht-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[searcht-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set SEARCHT_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[searcht-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[searcht-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[searcht-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[searcht-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[searcht-web] username: ${username}`);
    console.log(`[searcht-web] new password: ${newPassword}`);
    console.log('[searcht-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: searcht-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.searcht-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.searcht-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  SEARCHT_PORT, SEARCHT_ALLOW_REMOTE, SEARCHT_DATA_DIR, SEARCHT_LOG_DIR,
  SEARCHT_BACKEND_BIN, SEARCHT_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: searcht-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[searcht-web] fatal:', err.message);
