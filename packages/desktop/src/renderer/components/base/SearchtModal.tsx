...
/** SearchtModal 组件 Props */
export interface SearchtModalProps extends Omit<ModalProps, 'title' | 'footer'> {
...
const STD_HEADER_CLASS = 'searcht-modal-std-header flex items-start justify-between gap-16px px-24px pt-20px pb-16px';
...
const STD_BODY_LAYOUT_CLASS = 'searcht-modal-std-body min-h-0 flex-1 overflow-y-auto';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <SearchtModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </SearchtModal>
 *
 * // 预设尺寸 / Preset size
 * <SearchtModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </SearchtModal>
 *
...
const SearchtModal: React.FC<SearchtModalProps> = ({
  children,
  variant,
  size,
  header,
  ...
  const renderFooter = () => {
    ...
    if (footerConfig.render) {
      ...
      const footerClassName = classNames(
        ...
        isStandard && 'searcht-modal-std-footer',
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={classNames('searcht-modal', isStandard && 'searcht-modal-standard', className)}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div
        className={classNames('searcht-modal-wrapper', isStandard && 'flex flex-col min-h-0')}
        style={{ borderRadius: borderRadiusVal }}
      >
        {renderHeader()}
        <div
          className={classNames(
            'searcht-modal-body-content',
...
SearchtModal.displayName = 'SearchtModal';

export default SearchtModal;
