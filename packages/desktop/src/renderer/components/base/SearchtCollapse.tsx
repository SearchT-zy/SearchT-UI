 ...
 */
export interface SearchtCollapseProps {
 ...
 */
export interface SearchtCollapseItemProps {
 ...
 */
const SearchtCollapseItem: React.FC<SearchtCollapseItemProps> = ({ children }) => <>{children}</>;
SearchtCollapseItem.displayName = 'SearchtCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <SearchtCollapse defaultActiveKey={['1']}>
 *   <SearchtCollapse.Item name="1" header="面板1">
 *     内容1
 *   </SearchtCollapse.Item>
 *   <SearchtCollapse.Item name="2" header="面板2">
 *     内容2
 *   </SearchtCollapse.Item>
 * </SearchtCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <SearchtCollapse accordion defaultActiveKey="1">
 *   <SearchtCollapse.Item name="1" header="面板1">内容1</SearchtCollapse.Item>
 *   <SearchtCollapse.Item name="2" header="面板2">内容2</SearchtCollapse.Item>
 * </SearchtCollapse>
 *
 * // 自定义图标 / Custom icon
 * <SearchtCollapse
 ...
 * ```
 */
const SearchtCollapseComponent: React.FC<SearchtCollapseProps> & { Item: typeof SearchtCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<SearchtCollapseItemProps> => {
      return React.isValidElement(child) && child.type === SearchtCollapseItem;
    });
  }, [children]);
...
SearchtCollapseComponent.Item = SearchtCollapseItem;

export default SearchtCollapseComponent;
