...
export const UPDATE_AVAILABLE_EVENT = 'searcht-update-available';
...
const RELEASES_PAGE_URL = 'https://github.com/searcht-ui/SearchT-UI/releases';
...
export const useUpdateNotificationController = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('searcht-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('searcht-open-update-modal', handleWindowOpen);
