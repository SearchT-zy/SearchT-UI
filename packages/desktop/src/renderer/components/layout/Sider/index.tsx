...
const Sider: React.FC<SiderProps> = ({ onSessionClick, collapsed = false }) => {
  ...
  useEffect(() => {
    ...
    void loadWorkspacePreferences()
      .then((preferences) => {
        if (active) setWorkspacePreferences(preferences);
      })
      .catch(() => {
        if (active) setWorkspacePreferences(DEFAULT_WORKSPACE_PREFERENCES);
      });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    const syncPreferences = (event: Event) => {
      const next = (event as CustomEvent<WorkspacePreferences>).detail;
      if (next) setWorkspacePreferences(next);
    };
    window.addEventListener(WORKSPACE_PREFERENCES_CHANGED_EVENT, syncPreferences);
    return () => window.removeEventListener(WORKSPACE_PREFERENCES_CHANGED_EVENT, syncPreferences);
  }, []);
  ...
  const handlePersonalNavigate = (target: string) => {
    cleanupSiderTooltips();
    blurActiveElement();
    closePreview();
    setIsBatchMode(false);
