...
const Titlebar: React.FC<TitlebarProps> = ({ workspaceAvailable }) => {
  ...
  const appTitle = useMemo(() => SEARCHT_DISPLAY_NAME, []);
  ...
  useEffect(() => {
    if (!isSettingsRoute) {
      ...
      try {
        sessionStorage.setItem('searcht:last-non-settings-path', path);
      } catch {
        // ignore
      }
      return;
    ...
    try {
      const stored = sessionStorage.getItem('searcht:last-non-settings-path');
      if (stored) {
