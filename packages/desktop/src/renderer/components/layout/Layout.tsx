...
const Layout: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      window.dispatchEvent(new CustomEvent('searcht-open-update-modal', { detail: { source: 'tray' } }));
    };
              ...
              >
                ...
                {isSettingsRoute ? (
                  <Tooltip content={t('common.back', { defaultValue: 'Back to Chat' })} position='bottom'>
                    ...
                    >
                      {SEARCHT_DISPLAY_NAME}
                    </div>
                  </Tooltip>
                ) : (
                  <div className='text-16px text-t-primary collapsed-hidden font-semibold'>{SEARCHT_DISPLAY_NAME}</div>
                )}
                {isMobile && !collapsed && (
                  <button
                    type='button'
