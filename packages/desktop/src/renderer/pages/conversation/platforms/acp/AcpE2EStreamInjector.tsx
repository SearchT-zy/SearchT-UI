...
const ENABLED_CONVERSATION_KEY = 'searcht:e2e-message-stream-conversation-id';
...
declare global {
  interface Window {
    __SEARCHT_E2E_MESSAGE_STREAM__?: StreamRegistry;
  }
}
...
const AcpE2EStreamInjector: React.FC<{ conversationId: string }> = ({ conversationId }) => {
  ...
  useEffect(() => {
    ...
    const registry = (window.__SEARCHT_E2E_MESSAGE_STREAM__ ??= { controllers: {} });
    ...
    return () => {
      if (window.__SEARCHT_E2E_MESSAGE_STREAM__) {
        delete window.__SEARCHT_E2E_MESSAGE_STREAM__.controllers[conversationId];
      }
    };
