...
export type SearchtModelSelection = {
...
export type UseSearchtModelSelectionOptions = {
  initialModel: TProviderWithModel | undefined;
  onSelectModel: (provider: IProvider, modelName: string) => Promise<boolean>;
};

export const useSearchtModelSelection = ({
  initialModel,
  onSelectModel,
}: UseSearchtModelSelectionOptions): SearchtModelSelection => {
  ...
  // The backend does not support Google Auth — filter it out
