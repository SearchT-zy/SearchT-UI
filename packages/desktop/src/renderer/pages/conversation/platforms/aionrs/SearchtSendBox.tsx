...
import { useSearchtMessage } from './useSearchtMessage';
import type { SearchtModelSelection } from './useSearchtModelSelection';
...
const useSearchtSendBoxDraft = getSendBoxDraftHook('aionrs', {
  _type: 'aionrs',
  atPath: [],
  content: '',
  uploadFile: [],
...
const useSendBoxDraft = (conversation_id: string) => {
  const { data, mutate } = useSearchtSendBoxDraft(conversation_id);
...
const SearchtSendBox: React.FC<{
  conversation_id: string;
  modelSelection: SearchtModelSelection;
  session_mode?: string;
  agent_name?: string;
  ...
  const { thought, running, turnStartedAtMs, setActiveMsgId, setWaitingResponse, resetState } = useSearchtMessage(
    conversation_id,
    {
  ...
  useEffect(() => {
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        console.error('[SearchtSendBox] Failed to send initial message:', error);
  ...
  const handleSheetModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        console.error('[SearchtSendBox] Failed to switch mode via sheet:', error);
  ...
  const handleStop = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[SearchtSendBox] stop request failed', error);
            ...
            {uploadFile.length > 0 && (
              <HorizontalFileList>
                {uploadFile.map((path) => (
                  <FilePreview
                    key={path}
                    data-testid={`searcht-file-tag-${uploadFile.indexOf(path)}`}
                    path={path}
                ...
                {atPath.map((item) => {
                  ...
                  if (!item.isFile) {
                    ...
                    return (
                      <Tag
                        key={item.path}
                        data-testid={`searcht-folder-tag-${folderIndex}`}
                        color='blue'
                        closable
                        onClose={() => {
