...
import { assistantRuntimeKey, isSearchtAssistant, type Assistant } from '@/common/types/agent/assistantTypes';
...
export function pickDefaultAssistantSelectionKey(assistants: Assistant[]): string | null {
  const enabledAssistants = assistants.filter((assistant) => assistant.enabled !== false);
  const preferred =
    enabledAssistants.find((assistant) => assistant.source === 'generated' && isSearchtAssistant(assistant)) ??
    enabledAssistants.find((assistant) => isSearchtAssistant(assistant)) ??
    enabledAssistants[0];
