...
import { groupClient } from '../group/groupClient';
...
export function useTeamList() {
  ...
  useEffect(() => {
    ...
    const unsubRemoved = ipcBridge.team.removed.on((event) => {
      void groupClient
        .removeTeam(event.team_id)
        .catch((error: unknown) => {
          console.warn('[Renderer:teamList] collaboration_cleanup_failed', { teamId: event.team_id, error });
        })
        .finally(() => mutate());
    });
  ...
  const removeTeam = useCallback(
    async (id: string) => {
      ...
      await groupClient.removeTeam(id).catch((error: unknown) => {
        console.warn('[Renderer:teamList] collaboration_cleanup_failed', { teamId: id, error });
      });
