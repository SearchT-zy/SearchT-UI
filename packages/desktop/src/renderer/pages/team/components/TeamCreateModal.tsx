...
// 注意：迁移到 SearchtModal variant='standard' 后，关闭按钮为 button[aria-label="Close"]，
...
const TeamCreateModal: React.FC<Props> = ({ visible, onClose, onCreated }) => {
  ...
  const handleCreate = async () => {
    ...
    if (!hasOneLeader) {
      Message.warning(t('team.create.selectOneLeader', { defaultValue: 'Select one Coordinator' }));
      return;
    }
  ...
  const teamFields = (
    <div className='grid grid-cols-[76px_minmax(0,1fr)] items-center gap-x-14px gap-y-10px'>
      <div className='text-14px font-600 leading-21px text-t-secondary'>
        {t('team.create.nameLabel', { defaultValue: 'Group name' })}
        <span className='ml-4px text-danger-6'>*</span>
      </div>
      <div>
        <Input
          ref={nameInputRef}
          placeholder={t('team.create.namePlaceholder', { defaultValue: 'Group name' })}
          value={name}
          onChange={setName}
  ...
  return (
    <SearchtModal
      ...
      header={{
        title: t('team.create.title', { defaultValue: 'New Agent group' }),
