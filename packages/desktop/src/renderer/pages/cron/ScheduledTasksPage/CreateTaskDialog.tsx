...
interface CreateTaskDialogProps {
  ...
  initialDraft?: CreateTaskDialogInitialDraft;
  onCreated?: (job: ICronJob) => void | Promise<void>;
}

type FrequencyType = CreateTaskFrequency;
type CustomFrequencyMode = 'interval' | 'daily' | 'weekly' | 'monthly' | 'advanced';
type CustomIntervalUnit = 'minutes' | 'hours';
type ExecutionMode = CreateTaskExecutionMode;
...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  useEffect(() => {
    ...
    if (editJob) {
      ...
      const draft = initialDraft ? resolveCreateTaskInitialDraft(initialDraft) : null;
      form.resetFields();
      if (draft) form.setFieldsValue({ name: draft.name, prompt: draft.prompt });
      setFrequency(draft?.frequency ?? 'manual');
      setTime(draft?.time ?? '09:00');
      setWeekday(draft?.weekday ?? 'MON');
      setCustomSchedule(
        draft?.frequency === 'custom' ? parseCustomSchedule(initialDraft!.schedule.expr) : createDefaultCustomSchedule()
      );
      setExecutionMode(draft?.executionMode ?? 'new_conversation');
      setQueueEnabled(draft?.queueEnabled ?? false);
  ...
  }, [visible, editJob, form, initialDraft]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isEditMode) {
        ...
        const createdJob = await ipcBridge.cron.addJob.invoke(params);
