...
import type { ManagedSkill } from '@/common/types/searcht/skillConsolidation';
...
import SkillVersionHistory from './consolidation/SkillVersionHistory';
import { skillLifecycleClient } from './consolidation/skillLifecycleClient';
...
const SkillDetailPage: React.FC = () => {
  ...
  const {
    ...
    mutate: mutateSkills,
  } = useSWR<SkillInfo[]>('skills.list', () => ipcBridge.fs.listAvailableSkills.invoke());
  const {
    data: managedSkill,
    isLoading: managedSkillLoading,
    mutate: mutateManagedSkill,
  } = useSWR<ManagedSkill | null>(['skill-lifecycle.managed', decodedName], () =>
    skillLifecycleClient.getManagedSkill(decodedName)
  );
  const {
    data: assistants,
  ...
  const displaySkill: SkillInfo | null =
    skill ??
    (managedSkill
      ? {
  ...
  const loading = skillsLoading || assistantsLoading || managedSkillLoading;
          ...
          <div className='mx-auto flex w-full max-w-760px flex-col gap-16px'>
            ...
            >
              <div className='flex gap-16px'>
                <div
                  className={`h-48px w-48px shrink-0 rounded-12px flex items-center justify-center text-18px font-bold shadow-sm text-transform-uppercase ${getAvatarColorClass(displaySkill.name)}`}
                >
                  {displaySkill.name.charAt(0).toUpperCase()}
                </div>
                <div className='min-w-0 flex flex-col gap-6px'>
                  <div className='flex items-center gap-8px'>
                    <span className='text-16px font-600 text-t-primary'>{displaySkill.name}</span>
                    <span className='rounded-4px border border-border-2 bg-fill-1 px-6px py-1px text-11px text-t-secondary'>
                      {sourceLabel(displaySkill)}
                    </span>
                  </div>
            ...
            {managedSkill ? (
              <div className='px-2px py-4px'>
                <SkillVersionHistory
                  managedSkill={managedSkill}
                  onChanged={async (next) => {
                    await mutateManagedSkill(next, { revalidate: false });
                    await mutateSkills();
                  }}
                />
              </div>
