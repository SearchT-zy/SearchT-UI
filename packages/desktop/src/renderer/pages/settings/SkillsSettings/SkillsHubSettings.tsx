...
type SkillsTab = 'review' | 'custom' | 'official';

const getSkillsTabFromState = (state: unknown): SkillsTab => {
  if (typeof state === 'object' && state !== null && 'skillsTab' in state) {
    if (state.skillsTab === 'review' || state.skillsTab === 'official') return state.skillsTab;
  }
  return 'custom';
};
      ...
      <div data-testid='official-skills-section'>
        <p className='m-0 mb-12px text-12px leading-relaxed text-t-tertiary'>
          {t('settings.skillsHub.officialHint', {
            defaultValue: 'Built-in skills maintained by SearchT-UI — read-only and updated with each release.',
          })}
        </p>
    ...
    <div className='flex flex-col gap-16px'>
      <SettingsPageHeader
        ...
        actions={
          <>
            ...
            {!isMobile && activeTab !== 'review' && searchBox('input-search-my-skills')}
            <Button
              type='text'
              size='small'
        ...
        tabs={[
          {
            key: 'review',
            label: t('personal.skillConsolidation.tabReview', { defaultValue: 'Pending review' }),
      ...
      {activeTab === 'review' ? (
        <SkillCandidateList
          ...
          onOpen={(id) => void navigate(`/settings/skills/review/${encodeURIComponent(id)}`)}
        />
      ) : activeTab === 'custom' ? (
        customPane
      ) : (
