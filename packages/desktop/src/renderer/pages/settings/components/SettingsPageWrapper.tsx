...
export function getBuiltinSettingsNavItems(isDesktop: boolean, t: TranslateFn): NavItem[] {
  const builtinMap: Record<string, NavItem> = {
    ...
    'personal-workspace': {
      ...
      label: t('personal.settings.title', { defaultValue: '个人工作台' }),
      icon: <Dashboard theme='outline' size='16' />,
      path: 'personal-workspace',
    },
    connections: {
      id: 'connections',
      label: t('personal.connectors.title'),
      icon: <FolderOpen theme='outline' size='16' />,
      path: 'connections',
    },
    memory: {
      id: 'memory',
      label: t('personal.memory.title'),
...
const SettingsPageWrapper: React.FC<SettingsPageWrapperProps> = ({ children, className, contentClassName }) => {
  ...
  React.useLayoutEffect(() => {
    if (!isMobile) return;
    activeMobileTabRef.current?.scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'center' });
