...
export const invoke = <Data = unknown>(name: string, data?: unknown): Promise<Data> => {
  ...
  const errorCallbackName = `subscribe.callback.error-${name}${id}`;
  ...
  return new Promise<Data>((resolve, reject) => {
    ...
    const disposeSuccess = on(callbackName, (result) => {
      disposeAll();
      resolve(result as Data);
    });
    const disposeError = on(errorCallbackName, (result) => {
      disposeAll();
      const message =
