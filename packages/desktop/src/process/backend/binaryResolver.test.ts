...
describe('resolveBinaryPath', () => {
  ...
  it('attaches bundled path diagnostics when the backend binary cannot be resolved', () => {
    const resourcesPath = '/app/resources';
    const runtimeKey = `${process.platform}-${process.arch}`;
    const binaryName = process.platform === 'win32' ? 'searcht-backend.exe' : 'searcht-backend';
    ...
    vi.mocked(readdirSync).mockImplementation((path) => {
      if (path === resourcesPath) return [dirEntry('bundled-backend', true)];
    ...
    expect(() => resolveBinaryPath()).toThrow('Cannot find "searcht-backend" binary');
    ...
    try {
      ...
      expect(error).toMatchObject({
        ...
        diagnostics: expect.objectContaining({
          ...
          resourcesDirEntries: ['bundled-backend/'],
          runtimeDirEntries: ['manifest.json'],
          pathLookupCommand:
            process.platform === 'win32'
              ? 'where searcht-backend; where aioncore'
              : 'which searcht-backend; which aioncore',
