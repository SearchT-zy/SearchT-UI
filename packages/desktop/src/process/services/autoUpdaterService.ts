...
class AutoUpdaterService extends EventEmitter {
  ...
  constructor() {
    ...
    if (cdnFeedOptions) {
      ...
      log.info(`[auto-update] ${SEARCHT_UPDATE_NOT_CONFIGURED}`);
    }
  }
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        ...
        'updaterCacheDirName: cn.searcht.desktop',
        '',
      ].join('\n');
  ...
  private moveCwdOutOfInstallDirForWindowsHandoff(): void {
    ...
    try {
      const safeCwd = path.join(app.getPath('temp'), 'searcht-updater-cwd');
   ...
   * while the downloaded archive contains the packaged app (cn.searcht.desktop). Squirrel.Mac
  ...
  async checkForUpdates(): Promise<{ success: boolean; updateInfo?: UpdateInfo; error?: string }> {
    try {
      ...
      if (!isSearchtUpdateServiceConfigured()) {
        return { success: false, error: SEARCHT_UPDATE_NOT_CONFIGURED };
      }
