...
import { mkdirSync as _mkdirSync, existsSync, readdirSync, readFileSync, renameSync } from 'fs';
...
const STORAGE_PATH = {
  config: 'searcht-config.txt',
  chatMessage: 'searcht-chat-message.txt',
  chat: 'searcht-chat.txt',
  env: '.searcht-env',
...
// Pre-fork on-disk file names, renamed once on startup so existing installs keep their data.
const LEGACY_STORAGE_FILE_NAMES: Partial<Record<keyof typeof STORAGE_PATH, string>> = {
...
const migrateLegacyStorageFiles = (dir: string) => {
  for (const [key, legacyName] of Object.entries(LEGACY_STORAGE_FILE_NAMES)) {
    const legacyPath = path.join(dir, legacyName);
    const nextPath = path.join(dir, STORAGE_PATH[key as keyof typeof STORAGE_PATH]);
    try {
      if (existsSync(legacyPath) && !existsSync(nextPath)) renameSync(legacyPath, nextPath);
    } catch {
      // Non-fatal: JsonFileBuilder creates a fresh file when needed.
    }
  }
};
...
migrateLegacyStorageFiles(getHomePage());
...
migrateLegacyStorageFiles(cacheDir);
...
const CHAT_HISTORY_DIR_NAME = 'searcht-chat-history';
const LEGACY_CHAT_HISTORY_DIR_NAME = 'aionui-chat-history';

// One-time folder rename so pre-fork installs keep their chat history.
const migrateChatHistoryDir = (dir: string) => {
  const legacy = path.join(dir, LEGACY_CHAT_HISTORY_DIR_NAME);
  const next = path.join(dir, CHAT_HISTORY_DIR_NAME);
  if (existsSync(legacy) && !existsSync(next)) {
    try {
      renameSync(legacy, next);
    } catch {
      // Non-fatal: fall back to per-file copy on access.
    }
  }
};

const buildMessageListStorage = (conversation_id: string, dir: string) => {
  migrateChatHistoryDir(dir);
  const fullName = path.join(dir, CHAT_HISTORY_DIR_NAME, conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, CHAT_HISTORY_DIR_NAME));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, CHAT_HISTORY_DIR_NAME, conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, CHAT_HISTORY_DIR_NAME, 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
 ...
 * `crates/searcht-app/assets/builtin-assistants/` and
 * `crates/searcht-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  // 1b. Rename pre-fork storage files copied in by the migration above; the
  //     config/chat builders load lazily, so they pick up the renamed files.
  migrateLegacyStorageFiles(getHomePage());
  migrateLegacyStorageFiles(cacheDir);
  ...
  //    now live in the backend SQLite catalog (see searcht-assistant crate) and
