...
import { resolveSearchtStoragePaths, SEARCHT_BRAND } from '@/common/config/brand';
...
export const getTempPath = () => {
  ...
  return path.join(rootPath, SEARCHT_BRAND.dataDirectoryName);
};
 ...
 * Release builds use ~/.searcht; dev builds use ~/.searcht-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.searcht，Dev 模式使用 ~/.searcht-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const { dataPath } = resolveSearchtStoragePaths(rootPath);
  return ensureCliSafeSymlink(dataPath, getEnvAwareName(SEARCHT_BRAND.cliDataDirectoryName));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.searcht-config; dev builds use ~/.searcht-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.searcht-config，Dev 模式使用 ~/.searcht-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const { configPath } = resolveSearchtStoragePaths(rootPath);
  return ensureCliSafeSymlink(configPath, getEnvAwareName(SEARCHT_BRAND.cliConfigDirectoryName));
};
