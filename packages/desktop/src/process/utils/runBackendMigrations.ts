...
import { BUILTIN_BROWSER_MCP_NAME, BUILTIN_MEMORY_MCP_NAME } from '@/common/config/constants';
import {
...
import { getBuiltinMcpScriptPath, getSystemDir, type ProcessConfig as ProcessConfigType } from './initStorage';
...
const BUILTIN_MEMORY_SCRIPT = 'builtin-mcp-memory';
...
async function ensureBootstrapMcpServersInDb(configFile: ConfigFile): Promise<void> {
  ...
  const memoryServer = buildMemoryMcpServerConfig(
    getBuiltinMcpScriptPath(BUILTIN_MEMORY_SCRIPT),
    getSystemDir().workDir
  );
  const defaultServers = [...buildDefaultMcpServers(), memoryServer];
  ...
  const existingMemoryServer = existingByName.get(BUILTIN_MEMORY_MCP_NAME);
  if (
    existingMemoryServer &&
    (!isSameStdioTransport(existingMemoryServer.transport, memoryServer.transport) ||
      existingMemoryServer.builtin !== true ||
      existingMemoryServer.enabled !== true ||
      existingMemoryServer.original_json !== memoryServer.original_json)
  ) {
    await mcpService.updateServer.invoke({
      id: existingMemoryServer.id,
      data: {
        description: memoryServer.description,
        builtin: true,
        transport: memoryServer.transport,
        original_json: memoryServer.original_json,
      },
    });
  }
  if (existingMemoryServer && existingMemoryServer.enabled !== true) {
    await mcpService.toggleServer.invoke({ id: existingMemoryServer.id });
  }
