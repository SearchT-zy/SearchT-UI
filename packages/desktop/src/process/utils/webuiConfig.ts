...
import { startWebHost, type WebHostHandle } from '@searcht/web-host';
...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.SEARCHT_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.SEARCHT_ALLOW_REMOTE || process.env.SEARCHT_REMOTE);
  const hostHint = process.env.SEARCHT_HOST?.trim();
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns ~/.searcht[-dev] symlink on macOS to sidestep
