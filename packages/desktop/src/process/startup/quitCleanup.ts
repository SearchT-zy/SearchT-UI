...
type QuitCleanupDeps = {
  ...
  stopLocalFolderConnectorScheduler: () => Promise<void> | void;
  stopEmailConnectorScheduler: () => Promise<void> | void;
  stopWebDavConnectorScheduler: () => Promise<void> | void;
  stopReminderScheduler: () => void;
  closePersonalCore: () => void;
...
async function runQuitCleanup(deps: QuitCleanupDeps): Promise<void> {
  ...
  const cleanup = async () => {
    ...
    await Promise.all([
      deps.stopLocalFolderConnectorScheduler(),
      deps.stopEmailConnectorScheduler(),
      deps.stopWebDavConnectorScheduler(),
    ]);
    deps.stopReminderScheduler();
    deps.closePersonalCore();
