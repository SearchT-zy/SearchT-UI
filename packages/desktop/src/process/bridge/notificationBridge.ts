...
export async function showNotification({
  ...
  if (notificationEnabled === false) {
    ...
    if (failWhenDisabled) throw new Error('Notifications are disabled');
    return;
  }

  // Do not notify while the user is already looking at the app.
  if (!allowFocused && mainWindowRef && !mainWindowRef.isDestroyed() && mainWindowRef.isFocused()) {
...
export async function showCalendarNotification(title: string, body: string): Promise<void> {
  await showNotification({ title, body, allowFocused: true, failWhenDisabled: true });
}
