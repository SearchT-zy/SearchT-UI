...
function isBackendStartupFailureEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.['searcht.failure'] === 'backend_startup';
}

function isUserFeedbackEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.type === 'user-feedback' || event.tags?.['searcht.installation_integrity.user_report'] === 'true';
}
...
export async function captureBackendStartupFailure(error: unknown): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.setTag('searcht.failure', 'backend_startup');
    scope.setTag('searcht.backend_startup.reason', failureInfo.reason);
    if (failureInfo.runtime) {
      scope.setTag('searcht.backend_startup.runtime', failureInfo.runtime);
    }
    if (failureInfo.packageArch) {
      scope.setTag('searcht.backend_startup.package_arch', failureInfo.packageArch);
    }
    if (failureInfo.deviceArch) {
      scope.setTag('searcht.backend_startup.device_arch', failureInfo.deviceArch);
    }
    if (failureInfo.expectedDownloadArch) {
      scope.setTag('searcht.backend_startup.expected_download_arch', failureInfo.expectedDownloadArch);
    }
    if (typeof failureInfo.isRosettaTranslated === 'boolean') {
      scope.setTag('searcht.backend_startup.rosetta_translated', getBooleanTagValue(failureInfo.isRosettaTranslated));
    }
    if (typeof details?.stage === 'string') {
      scope.setTag('searcht.backend_startup.stage', details.stage);
    }
    if (failureInfo.backendBoundaryCode) {
      scope.setTag('searcht.backend_startup.boundary_code', failureInfo.backendBoundaryCode);
    }
    if (failureInfo.backendBoundaryStage) {
      scope.setTag('searcht.backend_startup.boundary_stage', failureInfo.backendBoundaryStage);
    }
    if (failureInfo.localDataIssueKind) {
      scope.setTag('searcht.backend_startup.local_data_issue_kind', failureInfo.localDataIssueKind);
    }
    if (failureInfo.incompleteInstallationKind) {
      scope.setTag('searcht.backend_startup.incomplete_installation_kind', failureInfo.incompleteInstallationKind);
    }
    for (const [tag, value] of [
      ['searcht.backend_startup.missing_bundled_dir', getBooleanTagValue(failureInfo.missingBundledBackendDir)],
      ['searcht.backend_startup.missing_runtime_dir', getBooleanTagValue(failureInfo.missingRuntimeDir)],
      ['searcht.backend_startup.missing_binary', getBooleanTagValue(failureInfo.missingBackendBinary)],
      ['searcht.backend_startup.missing_hub_dir', getBooleanTagValue(failureInfo.missingHubDir)],
      ['searcht.backend_startup.missing_pet_states_dir', getBooleanTagValue(failureInfo.missingPetStatesDir)],
      ['searcht.backend_startup.missing_pwa_dir', getBooleanTagValue(failureInfo.missingPwaDir)],
      ['searcht.backend_startup.install_path_kind', getInstallPathKind(details?.resourcesPath)],
      ['searcht.backend_startup.last_update_status', getString(autoUpdateDiagnostics?.lastEvent?.status)],
      [
        'searcht.backend_startup.health_polling_delayed',
        getBooleanTagValue(
          typeof details?.healthCheckPollingDelayed === 'boolean' ? details.healthCheckPollingDelayed : undefined
        ),
      ],
      ['searcht.backend_startup.health_attempts_bucket', getHealthAttemptBucket(details?.healthCheckAttempts)],
      [
        'searcht.backend_startup.health_attempt_deficit_bucket',
        getHealthAttemptBucket(details?.healthCheckAttemptDeficit),
      ],
      [
        'searcht.backend_startup.health_timeout_overrun_bucket',
        getDurationBucket(details?.healthCheckTimeoutOverrunMs),
      ],
      ['searcht.backend_startup.health_max_attempt_gap_bucket', getDurationBucket(details?.healthCheckMaxAttemptGapMs)],
      [
        'searcht.backend_startup.seconds_since_quit_and_install',
    ...
    if (details) {
      scope.setContext('searcht.backend_startup', details);
      scope.setExtra('searcht.backend_startup', details);
    }
    scope.setContext('searcht.backend_startup_classification', { ...failureInfo });
    scope.setExtra('searcht.backend_startup_classification', failureInfo);
    scope.setContext('searcht.backend_install_diagnostics', installDiagnostics);
    scope.setExtra('searcht.backend_install_diagnostics', installDiagnostics);
