...
import {
  ...
  stopLocalFolderSyncScheduler,
  stopPersonalReminderScheduler,
  stopWebDavSyncScheduler,
...
import { BackendLifecycleManager } from '@searcht/web-host';
...
import { startWebHost } from '@searcht/web-host';
...
const isE2ETestMode = process.env.SEARCHT_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.SEARCHT_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[SearchT-UI] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[SearchT-UI] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      method: 'POST',
      headers: {
        'x-searcht-internal': '1',
      },
    }).catch((error) => {
      console.error('[SearchT-UI] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[SearchT-UI] runBackendMigrations completed');
    } catch (error) {
      console.error('[SearchT-UI] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[SearchT-UI] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  const reason = process.env.SEARCHT_DEBUG_BACKEND_STARTUP_FAILURE as BackendStartupFailureInfo['reason'] | undefined;
  if (!reason) {
    return null;
  }
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[SearchT-UI] Ignoring SEARCHT_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[SearchT-UI] Ignoring unknown SEARCHT_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[SearchT-UI] Creating main window...');
  const { x: windowX, y: windowY, width: windowWidth, height: windowHeight } = resolveInitialBounds();

  // Load the app icon for the BrowserWindow on Windows/Linux (macOS uses
  // the dock badge). In packaged builds the PNG ships next to app.asar;
  // in dev it comes from the repo's resources/ directory.
  let appIcon: Electron.NativeImage | undefined;
  try {
    const iconCandidates = app.isPackaged
      ? [path.join(process.resourcesPath, 'app.png')]
      : [
          path.join(process.cwd(), 'resources', process.platform === 'win32' ? 'app.ico' : 'app_dev.png'),
  ...
  console.log(`[SearchT-UI] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[SearchT-UI] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[SearchT-UI] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[SearchT-UI] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  const disableAutoUpdater =
    process.env.SEARCHT_DISABLE_AUTO_UPDATE === '1' || process.env.SEARCHT_E2E_TEST === '1' || isCiRuntime;
  if (!disableAutoUpdater) {
    Promise.all([import('./process/services/autoUpdaterService'), import('./process/bridge/updateBridge')])
      .then(([{ autoUpdaterService }, { createAutoUpdateStatusBroadcast }]) => {
        ...
        // build: SearchT-UI's final version guides users to the website instead of
    ...
    console.log('[SearchT-UI] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[SearchT-UI] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[SearchT-UI] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[SearchT-UI] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[SearchT-UI] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[SearchT-UI] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[SearchT-UI] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[SearchT-UI] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[SearchT-UI] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[SearchT-UI] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[SearchT-UI] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[SearchT-UI] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[SearchT-UI] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[SearchT-UI:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
   ...
   *
   * 两个值靠进程继承链传给 Agent：后端（searcht-backend）是本进程的子进程，内置浏览器 MCP 又是
   * 后端的子进程，所以不用落库、不用写配置。但继承是「spawn 那一刻的快照」：
   * backend-launcher 用 `{ ...process.env }` spawn 后端，此后我们再改 process.env
   * 对已经起来的后端毫无影响。一旦这段挪到 backend 启动之后，后端继承到的
   ...
   * style. Both values reach the agent by process inheritance (the backend is our child; the
   * in-app browser MCP is the backend's child), so neither is persisted. But inheritance is a
   * snapshot taken at spawn time: backend-launcher spawns the backend with
   * `{ ...process.env }`, and any later mutation of our process.env is invisible to the
   * already-running backend. Move this block after backend startup and the backend inherits
  ...
  if (cdpStartupEnabled) {
    try {
      ...
      process.env.SEARCHT_CDP_ACTIVE_PORT = String(bridge.port);
      process.env.SEARCHT_CDP_BRIDGE_TOKEN = bridge.token;
  ...
  if (debugBackendStartupFailure) {
    ...
    // Start the backend only after initializeProcess(). initStorage may open
  ...
  try {
    ...
    console.error('[SearchT-UI] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[SearchT-UI] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`SearchT-UI --webui` or packaged `searcht-web` mode that
      ...
      const handle = await startWebHost({
        ...
        // Expose the same SEARCHT_{CACHE,WORK,LOG}_DIR env the desktop IPC path
        ...
        backend: {
          ...
          port: (() => {
            ...
            if (!port) {
              throw new Error('[WebUI] Cannot start: searcht-backend is not running (globalThis.__backendPort unset)');
            }
            return port;
          })(),
        },
...
// Register searcht:// as the default protocol client
...
// macOS: handle searcht:// URLs via the open-url event
...
// instance lock. A lock-losing instance must NOT spawn a competing backend
// process — doing so races the first instance's backend over the same data
...
if (shouldRegisterBackendStartup(gotTheLock)) {
  void app
    ...
    .catch((error) => {
      ...
      console.error('[SearchT-UI] App initialization failed:', error);
      app.quit();
    });
}
...
installQuitCleanup({
  ...
  stopLocalFolderConnectorScheduler: stopLocalFolderSyncScheduler,
  stopEmailConnectorScheduler: stopEmailSyncScheduler,
  stopWebDavConnectorScheduler: stopWebDavSyncScheduler,
  stopReminderScheduler: stopPersonalReminderScheduler,
  closePersonalCore,
  // Stop the backend subprocess — backend shutdown kills all agent children
...
app.on('will-quit', () => {
  console.log('[SearchT-UI] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[SearchT-UI] quit (exitCode=${exitCode})`);
});
