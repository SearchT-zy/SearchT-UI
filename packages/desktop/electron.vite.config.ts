...
// Read the real SearchT version from the repo-root package.json.
  ...
  return {
    main: {
      ...
      define: {
        ...
        // Discontinued-build fork flag (see discontinuedBuild.ts). Only SearchT's
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another SearchT clone is running),
      ...
      define: {
        ...
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real SearchT version (root package.json) so renderer code
