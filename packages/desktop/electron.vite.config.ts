...
// Read the real SearchT-UI version from the repo-root package.json.
  ...
  return {
    main: {
      plugins: [
        ...
        // '@searcht/web-host' excluded so its TS sources are bundled by esbuild
        // rather than left as `require('@searcht/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@searcht/web-host'] }),
        ...(isDevelopment
          ? [
              {
      ...
      define: {
        ...
        // Discontinued-build fork flag (see discontinuedBuild.ts). Only SearchT-UI's
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another SearchT-UI clone is running),
      ...
      define: {
        'process.env.NODE_ENV': JSON.stringify(mode),
        'process.env.env': JSON.stringify(process.env.env),
        'process.env.SEARCHT_MULTI_INSTANCE': JSON.stringify(process.env.SEARCHT_MULTI_INSTANCE ?? ''),
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real SearchT-UI version (root package.json) so renderer code
